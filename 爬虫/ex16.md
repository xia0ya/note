# 练习十六：复杂网页数据提取与XPath高级用法

本练习演示如何使用XPath高级用法，批量提取豆瓣电影信息，包括标题、图片、评分、导演、主演、类型、制片国家、语言、上映日期、片长等。

---

```python
# coding=utf-8
# 功能：批量提取豆瓣电影信息，演示XPath高级用法
...
def parseHTML(html):
    ...
    for div in divs:
        if i % 2 == 0:
            ...
            print('图片链接:', img_url)
            # downloadImage(img_url, './h02')  # 可选：下载图片
...
if __name__ == '__main__':
    html = getHTML(base_url, '.h02.html')
    parseHTML(html)
```
...
## 说明
- 该代码演示了如何用XPath高级用法批量提取复杂网页中的多种信息。
